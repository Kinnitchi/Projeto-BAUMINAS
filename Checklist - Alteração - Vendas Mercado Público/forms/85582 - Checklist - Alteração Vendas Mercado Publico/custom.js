...
$(function () {
	...
	$('#numeroFluig').on('change', function () {
		...
		if (dataset.values.length > 0) {
			...
			if (type == "ALTERACAO") {
				...
				if (alteracao.values.length > 0) {
					...
					if (control) {
						...
						FLUIGC.toast({
							title: 'Atenção!',
							message: 'Já existe uma solicitação em aberto para este número de pedido',
							type: 'info'
						});

						$('#UltimaSolicitacao').html('Ultima solicitação: <a href="' + parent.WCMAPI.tenantURL + '/pageworkflowview?app_ecm_workflowview_detailsProcessInstanceID=' + dsValues.at(-1).solicitacao + '" target="_blank" rel="noopener noreferrer "><strong class="text-info">' + dsValues.at(-1).solicitacao + '<strong></a>');
					...
					if (control) {
						...
						$('#UltimaSolicitacao').html('Ultima solicitação: <a href="' + parent.WCMAPI.tenantURL + '/pageworkflowview?app_ecm_workflowview_detailsProcessInstanceID=' + dsValues2.at(-1).solicitacao + '" target="_blank" rel="noopener noreferrer "><strong class="text-info">' + dsValues2.at(-1).solicitacao + '<strong></a>');
			...
			FLUIGC.toast({
				title: 'Atenção',
				message: 'Número de solicitação não encontrado.',
				type: 'warning'
			});
		}
...
const resetValues = () => {
	...
	for (let i = 0; i < fields.length; i++) {
		...
		fields[í].id.indexOf('numeroFluig') > -1 ? fields[i].value = fields[i].value : fields[i].value = '';
		fields[i].value = ''
...
function buscaHistorico(processId) {
	...
	if (dsHist1.values.length == 0) {
		...
		if (vendasDs.values.length > 0) {
			...
			var dsHistVal = "<span><u><b><a target='_blank' href='https://bauminashom.fluig.com:9100/portal/p/01/pageworkflowview?app_ecm_workflowview_detailsProcessInstanceID=" + processId + "'>Solicitação principal " + processId + "</a></b> feita em " + dsValues[0]["dataSolicitacao"] + "</u></span><br>";
