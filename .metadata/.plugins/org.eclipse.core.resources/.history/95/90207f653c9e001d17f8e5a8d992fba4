...
$(function () {
	...
	if ($("#atividade").val() == "" || $("#atividade").val() == "0" || $("#atividade").val() == "4") {
		...
		$("input[value='Mudou'][id$='Hidden']").each(function (i) {
			$(this).parent().addClass('has-warning');
			var idCampo = (this.id).replace("Hidden", "");
			$("#" + idCampo).addClass('corFundo');
			var valorOriginal = $('#' + idCampo + 'Original').val();
			const element = document.getElementById(idCampo);
			element.title = "Valor Original: " + valorOriginal;
		})
	} else {
		buscaHistorico();
	}
...
const verifyCheckbox = () => {
	...
	var arr = $("form[name='form']").find("input[type='checkbox']");

	for (let index = 0; index < arr.length; index++) {

		$('#' + arr[index].id + 'valOri').val(arr[index].checked.toString())
