function createDataset(fields, constraints, sortFields) {
    ...
    var arrConstra = []
    if (numeroSolicitacao) {
        arrConstra.push(DatasetFactory.createConstraint("textbox40", numeroSolicitacao, numeroSolicitacao, ConstraintType.MUST));
    } else {
        arrConstra.push(DatasetFactory.createConstraint("textbox40", "", "", ConstraintType.MUST_NOT));
        arrConstra.push(DatasetFactory.createConstraint("textbox40", null, null, ConstraintType.MUST_NOT));
        arrConstra.push(DatasetFactory.createConstraint("textbox40", "0", "0", ConstraintType.MUST_NOT));
    }
